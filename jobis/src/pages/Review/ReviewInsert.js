...
import styles from "./ReviewInsert.module.css"; // CSS 모듈을 통한 스타일링
...
const ReviewInsert = () => {
  ...
  const handleSubmit = async () => {
    ...
    // UUID와 사용자 정보 확인 로그
    console.log("ReviewInsert - 작성자 이름:", username);
    console.log("ReviewInsert - UUID:", uuid);
    ...
    try {
      ...
      const response = await axios.post(
        "http://localhost:8080/review", // 서버 엔드포인트 URL
        formData,
        {
          headers: {
      ...
      if (response.status === 201) {
        alert("리뷰이 등록되었습니다.");
        navigate("/review"); // Review 목록 페이지로 이동
      }
    } catch (error) {
      // 요청 실패 처리
