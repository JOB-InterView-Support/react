  ...
  useEffect(() => {
    async function fetchSelfIntroduceData() {
      ...
      try {
        ...
        const response = await secureApiRequest(`/mypage/self-introduce/${introNo}`, {
          method: "GET",
          headers: {
        ...
        console.error("자기소개서 데이터를 가져오는 중 오류 발생:", err.message);
      ...
      {loading ? (
        ...
        <div className={styles.selfIntroduce}>
          ...
          <div className={styles.field}>
            ...
            <p className={styles.value}>{selfIntroduceData.applicationCompanyName}</p>
          </div>
          <div className={styles.field}>
