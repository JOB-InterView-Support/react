...
function MyIntroductionDetail() {
  ...
  useEffect(() => {
    const fetchDetail = async () => {
      try {
        ...
        console.error("자기소개서 상세 데이터를 가져오는 중 오류 발생:", err.message);
  ...
  return (
    <div className={styles.container}>
      <h1 className={styles.title}>자기소개서 상세 보기 {detail.introTitle}</h1>
      <div className={styles.detailCard}>
        <p><strong>지원 회사명:</strong> {detail.applicationCompanyName}</p>
        <p><strong>지원 직무:</strong> {detail.workType}</p>
        <p><strong>보유 자격증:</strong> {detail.certificate}</p>
